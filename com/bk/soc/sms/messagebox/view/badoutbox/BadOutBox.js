...
row1.add({tag: 'div', html: '发送时间：',bodyStyle:queryLabel+'width:170'});
...
row2.add({tag: 'div', html: '收信人号码：',bodyStyle:queryLabel+'width:170'});
...
function appInit()
{
	...
	page1.add(mainArea);
	mainArea.refreshFor(mainArea.queryBadOutBox);
	//---------------------------------------------
	page2.init();
	page2.add(editArea);
...
//-----------------------------------------------------
//-----主页面查询
mainArea.queryBadOutBox=function()
{
	SMS_SmsService.queryBadOutBoxs(mainArea.getAreaInfo(),function(viewData)
	{
		mainArea.showViewData(viewData);
	});
}
//-----主页面删除
mainArea.deleteBadOutBox=function(constParam)
{
	...
	Ext.confirm('你确定删除这'+list.length+'条短信吗？',function(btn)
	{
		if(btn=='yes')
		{
			SMS_SmsService.opDeleteBadOutBox(list,function(viewData)
			{
...
editArea.editSend=function()
{
	...
	SMS_SmsService.opEditSend(phone,obj.msg,function(viewData)
	{
		if(!viewData.isSucceed)
		{
			Ext.alert('发送失败.'+viewData.message);
